...
public class ThreadBruteDomainWithDNSServer2{
    private String rootDomain;
    private List<DomainBruteProducerWithDNSServer2> plist;
    ...
    public ThreadBruteDomainWithDNSServer2(String rootDomain) {
...
/*        //TODO 不应该全删除，应当记录泛解析的IP。
        ...
        plist = new ArrayList<DomainBruteProducerWithDNSServer2>();

        RootDomainForBrute rd= new RootDomainForBrute(rootDomain);

        for (int i=0;i<=10000;i++) {
            DomainBruteProducerWithDNSServer2 p = new DomainBruteProducerWithDNSServer2(rd,dictQueue,outputQueue,i);
            p.start();
            plist.add(p);
        }
...
//            }//尝试阶段性保存
    ...
    boolean isAllProductorFinished(){
        for (DomainBruteProducerWithDNSServer2 p:plist) {
            if(p.isAlive()) {
                return false;
            }
        }
...
class DomainBruteProducerWithDNSServer2 extends Thread {//Producer do
    private RootDomainForBrute rootDomainForBrute;
    ...
    public DomainBruteProducerWithDNSServer2(RootDomainForBrute rootDomainForBrute,
        ...
        this.rootDomainForBrute = rootDomainForBrute;
        stopflag= false;
    ...
    public void run() {
        while(true) try {
            ...
            String tmpDomain = subdomainWord + "." + rootDomainForBrute.rootDomain;
            //stdout.println(tmpDomain);
            HashMap<String, Set<String>> result = rootDomainForBrute.query(tmpDomain);
            Set<String> CDNSet = result.get("CDN");
            Set<String> IPset = result.get("IP");
            boolean successfulflag = false;

            //根据CDN进行判断是否为泛解析域名
            if (CDNSet != null && CDNSet.size() != 0 && !CDNSet.toString().equalsIgnoreCase(rootDomainForBrute.wildCDNSet.toString())) {
                successfulflag = true;
            } else if (IPset != null && IPset.size() != 0) {//根据IP集合进行判断是否为泛解析域名
                Set<String> tmpSet = new HashSet<>();
                tmpSet.addAll(IPset);
                tmpSet.retainAll(rootDomainForBrute.wildIPset);
